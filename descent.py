...
def mirror_descent(model, X_train, y_train, param_name, impact, lr, eta, lambda_value, num_steps, criterion, start=None, error=None):
...
def gradient_descent(model, X_train, y_train, param_name, impact, lr, eta, num_steps, criterion, start=None, scale=1.0, error=None):
...
def mirror_descent_full(model, X_train, y_train, impacts, lr, eta, lambda_value, num_steps, criterion, start=None, errors=None):
...
def gradient_descent_full(model, X_train, y_train, impacts, lr, eta, num_steps, criterion, start=None, scale=1.0, errors=None):
    # 1) считаем один раз начальный градиент по весам
    outputs = model(X_train)
    loss = criterion(outputs, y_train)
    base_grads = torch.autograd.grad(loss, [p for _, p in model.named_parameters()])
